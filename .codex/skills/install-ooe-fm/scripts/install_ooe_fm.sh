#!/bin/bash
# ooe-fm Repository Installation Script
#
# This script clones/updates the ooe-fm (One Of Everything) FileMaker reference
# repository for XML test cases.
#
# Usage: install_ooe_fm.sh [--force]
#
# Parameters:
...
DOCS_DIR="$PROJECT_ROOT/docs/ooe-fm"
VERSION_FILE="$DOCS_DIR/.version"
REPO_URL="https://github.com/mislavkos/ooe-fm.git"
...
check_version() {
    if [ ! -d "$DOCS_DIR/.git" ]; then
        echo "No existing installation found. Cloning ooe-fm repository..."
        return 0
    fi

    if [ "$FORCE_INSTALL" = true ]; then
        echo "Force installation requested. Re-cloning ooe-fm repository..."
        return 0
    fi
...
install_repo() {
    ...
    if [[ ! "$DOCS_DIR" == *"/docs/ooe-fm" ]]; then
        echo "ERROR: DOCS_DIR does not match expected pattern (/docs/ooe-fm). Aborting for safety."
...
get_stats() {
    if [ -d "$DOCS_DIR" ]; then
        XML_COUNT=$(find "$DOCS_DIR" -name "*.xml" 2>/dev/null | wc -l | tr -d ' ')
        FMP12_COUNT=$(find "$DOCS_DIR" -name "*.fmp12" 2>/dev/null | wc -l | tr -d ' ')
        echo "($XML_COUNT XML files, $FMP12_COUNT fmp12 files)"
    fi
}
...
main() {
    ...
    LOCAL_DATE=$(get_local_date)
    echo ""
    echo "SUCCESS: ooe-fm repository installed successfully"
